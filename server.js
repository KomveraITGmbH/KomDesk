...
async function deactivateLicense() {
    ...
    if (data?.machineId && data?.key) {
        try {
            await keygenFetch(
                `https://api.keygen.sh/v1/accounts/${KEYGEN_ACCOUNT}/machines/${data.machineId}`,
                { method: 'DELETE', headers: { 'Authorization': `License ${data.key}` } }
            );
            console.log('[Lizenz] Machine bei Keygen deaktiviert');
        } catch (err) {
            console.warn('[Lizenz] Deaktivierung Netzwerkfehler:', err.message);
        }
    }
    try { fs.unlinkSync(LICENSE_FILE); } catch {}
    licenseValid = false;
    console.log('[Lizenz] Lizenz lokal entfernt');
    return { success: true };
}
...
==================================================
*/
const GITHUB_PKG_URL = 'https://raw.githubusercontent.com/KomveraITGmbH/KomDesk/main/package.json';
    ...
    return `
        <div class="support-footer">
            ...
            <div>
                ...
                <a href="mailto:komdesk@komvera.de">komdesk@komvera.de</a>
            </div>
        </div>
    `;
}
    ...
    return `
        <aside class="sidebar">
            <div class="sidebar-brand">
                ...
                <span class="brand-text">KomDesk Admin</span>
            </div>
...
function renderAdminLayout(req, title, content) {
    ...
    <head>
        ...
        <title>${escapeHtml(title)} – KomDesk Admin</title>
        ...
        <script>
            (function() {
                var saved = localStorage.getItem('komdesk-theme') || 'light';
    ...
    <body>
        ...
        <div class="layout">
            ...
            <main class="main">
                <div class="mobile-topbar">
                    ...
                    <span class="mobile-topbar-title">KomDesk Admin</span>
        ...
        <script>
            function toggleTheme() {
                ...
                localStorage.setItem('komdesk-theme', next);
            }

            window._devMode = localStorage.getItem('komdesk-devmode') === '1';
            ...
            function toggleDevMode() {
                ...
                localStorage.setItem('komdesk-devmode', window._devMode ? '1' : '0');
                applyDevMode();
            }
...
<title>Lizenz nicht aktiv – KomDesk</title>
<script>(function(){var s=localStorage.getItem('komdesk-theme')||'light';document.documentElement.setAttribute('data-theme',s);})();</script>
<style>
...
<div class="card">
  <div class="logo">
    ${_logoExists ? `<img src="/logo.png" alt="KomDesk">` : `<span class="logo-text">KomDesk</span>`}
  </div>
  <div class="icon">🔒</div>
...
<script>
  function toggleTheme(){
    ...
    localStorage.setItem('komdesk-theme',t);
  }
</script>
</body>
</html>`);
...
app.post('/admin/license/deactivate', express.urlencoded({ extended: false }), requireAdmin, requirePermission('license.manage'), requireCsrf, async (req, res) => {
    await deactivateLicense();
    return res.redirect('/admin/system?licenseDeactivated=1');
});
...
==================================================
*/
function renderLegalPage(title, filePath, res) {
    let content = '';
    try { content = fs.readFileSync(filePath, 'utf8'); } catch(e) { return res.status(404).send('Nicht gefunden'); }
    const logoExists = fs.existsSync(LOGO_FILE);
    const html = content
    ...
    <head>
        ...
        <style>
            ...
            *, *::before, *::after { box-sizing: border-box; }
            ...
            .header-logo-text { font-size: 20px; font-weight: 700; color: var(--text); text-decoration: none; }
            .btn-theme {
                background: none; border: 1px solid var(--border); border-radius: 8px;
                padding: 7px 14px; cursor: pointer; color: var(--muted); font-size: 14px;
                transition: border-color 0.15s, color 0.15s;
            }
            .btn-theme:hover { border-color: var(--primary); color: var(--primary); }
            main { flex: 1; max-width: 820px; width: 100%; margin: 0 auto; padding: 48px 24px 80px; }
            .back-link {
            ...
            .legal-card h1 { font-size: 1.7rem; font-weight: 700; margin: 0 0 6px; color: var(--text); }
            .legal-card h2 {
                font-size: 1.05rem; font-weight: 600; margin: 36px 0 10px;
                padding-bottom: 8px; border-bottom: 1px solid var(--border); color: var(--text);
            }
            .legal-card h3 { font-size: 0.95rem; font-weight: 600; margin: 20px 0 6px; color: var(--text); }
            .legal-card p { line-height: 1.75; margin: 8px 0; color: var(--text); }
            .legal-card hr { border: none; border-top: 1px solid var(--border); margin: 28px 0; }
    ...
    <body>
        <header>
            <a href="/">
                ${logoExists
                    ? `<img src="/logo.png" alt="KomDesk" class="header-logo">`
                    : `<span class="header-logo-text">KomDesk</span>`}
            </a>
            <button class="btn-theme" onclick="(function(){var c=document.documentElement.getAttribute('data-theme');var n=c==='dark'?'light':'dark';document.documentElement.setAttribute('data-theme',n);localStorage.setItem('komdesk-theme',n);})()">&#9790; Modus</button>
        </header>
        <main>
        ...
        <footer>
            ...
            <a href="/legal/agb">AGB</a> &middot;
            <a href="/legal/datenschutz">Datenschutz</a> &middot;
            <a href="/legal/eula">EULA</a> &middot;
...
app.get('/legal/agb',         (_, res) => renderLegalPage('AGB',                  path.join(__dirname, 'legal/agb.md'),                   res));
app.get('/legal/datenschutz', (_, res) => renderLegalPage('Datenschutzerklärung', path.join(__dirname, 'legal/datenschutzerklaerung.md'), res));
app.get('/legal/eula',        (_, res) => renderLegalPage('EULA',                 path.join(__dirname, 'legal/eula.md'),                  res));
...
app.get('/admin/setup', (req, res) => {
    ...
    res.send(`
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>${escapeHtml(L.setup?.title || 'Initial Setup')} – KomDesk</title>
        ...
        <body>
            <div class="page-wrap">
                <div class="setup-header">
                    <div class="setup-logo">
                        ${logoExists
                            ...
                            : `<span class="setup-logo-text">KomDesk</span>`
                        }
                    </div>
                ...
                <form method="POST" action="/admin/setup">
                    ...
                    <label style="display:flex;align-items:flex-start;gap:10px;font-size:13px;color:var(--text-muted,#64748b);margin-bottom:16px;cursor:pointer;">
                        <input type="checkbox" id="accept_legal" name="accept_legal" value="1" style="margin-top:2px;width:16px;height:16px;flex-shrink:0;">
                        <span>Ich habe die <a href="/legal/agb" target="_blank" style="color:var(--primary);">AGB</a>, die <a href="/legal/datenschutz" target="_blank" style="color:var(--primary);">Datenschutzerklärung</a> und die <a href="/legal/eula" target="_blank" style="color:var(--primary);">EULA</a> gelesen und akzeptiere diese.</span>
                    </label>
            ...
            <script>
                ...
                function validateSetup() {
                    ...
                    if (!document.getElementById('accept_legal').checked) { err.textContent = '\u2717 Bitte AGB, Datenschutzerklärung und EULA akzeptieren.'; err.style.display = ''; return false; }
    ...
    try {
        ...
        return res.send(`
            ...
            <head>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <title>Setup abgeschlossen – KomDesk</title>
                <style>
                    * { box-sizing: border-box; margin: 0; padding: 0; }
                    :root {
                        --bg: #f0f4f8;
            ...
            <body>
                <button class="theme-btn" onclick="(function(){var c=document.documentElement.getAttribute('data-theme');var n=c==='dark'?'light':'dark';document.documentElement.setAttribute('data-theme',n);localStorage.setItem('komdesk-theme',n);})()">&#9790; Modus</button>
                <div class="card">
                    <div class="icon">&#10003;</div>
...
app.get('/admin/login', (req, res) => {
    ...
    res.send(`
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>${escapeHtml(L.login?.title || 'Admin Login')} – KomDesk</title>
            <script>window._LOCALE=${JSON.stringify(L)};</script>
            <script>(function(){var L=window._LOCALE||{};function t(k){return k.split('.').reduce(function(o,p){return o&&o[p]},L)||k;}document.addEventListener('DOMContentLoaded',function(){document.querySelectorAll('[data-i18n]').forEach(function(el){var v=t(el.getAttribute('data-i18n'));if(v!==el.getAttribute('data-i18n'))el.textContent=v;});document.querySelectorAll('[data-i18n-placeholder]').forEach(function(el){var v=t(el.getAttribute('data-i18n-placeholder'));if(v!==el.getAttribute('data-i18n-placeholder'))el.placeholder=v;});});})();</script>
            <style>
                * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        <body>
            ...
            <div class="box">
                <div class="login-logo">
                    ${logoExists
                        ? `<img src="/logo.png" alt="KomDesk">`
                        : `<span class="login-logo-text">KomDesk</span>`
                    }
                </div>
...
app.get('/admin', requireAdmin, requirePermission('dashboard.view'), (req, res) => {
    ...
    const serverHtml = hasPermission(req, 'server.view') ? `
        <div class="card db-hero" style="grid-column:1/-1;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
                <div class="db-card-title" data-i18n="dashboard.komdeskServer" style="margin:0;">⚙️ KomDesk Server</div>
                <div style="display:flex;gap:8px;flex-wrap:wrap;" data-csrf="${getCsrfToken(req)}">
                    ...
                    ${hasPermission(req, 'server.restart') ? `<button onclick="srvConfirmRestart('komdesk')" style="padding:6px 14px;border:1.5px solid #f59e0b;border-radius:8px;background:transparent;color:#f59e0b;font-size:12px;font-weight:600;cursor:pointer;" data-i18n="dashboard.restartKomDesk">↺ KomDesk neu starten</button>` : ''}
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:16px;">
                ...
                <div>
                    <div style="font-size:12px;font-weight:600;opacity:.45;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px;" data-i18n="dashboard.processSection">KomDesk Prozess</div>
                    <div style="display:flex;flex-direction:column;gap:7px;">
                ...
                <div style="grid-column:1/-1;">
                    ...
                    <div id="srv-systemctl-wrap" style="display:none;margin-top:12px;">
                        <div style="font-size:12px;font-weight:600;opacity:.45;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px;">systemctl status komdesk</div>
    ...
    const licDeactivated  = req.query.licenseDeactivated === '1';
    const licenseHtml = (() => {
        if (!licenseValid) {
            return `<div class="card">
                <div class="db-card-title">🔑 Lizenz</div>
                <div style="display:flex;align-items:center;gap:10px;padding:12px 14px;background:#fef2f2;border:1px solid #fecaca;border-radius:10px;margin-bottom:16px;">
                    <span style="font-size:22px;">🔒</span>
                    <div>
                        <div style="font-weight:700;color:#dc2626;font-size:14px;">Nicht lizenziert</div>
                        <div style="font-size:12px;color:#7f1d1d;margin-top:2px;">Alle Funktionen gesperrt</div>
                    </div>
                </div>
                ${licError ? `<div style="background:#fee2e2;border:1px solid #fca5a5;color:#b91c1c;border-radius:7px;padding:8px 12px;font-size:12px;margin-bottom:12px;">${escapeHtml(licError)}</div>` : ''}
                <form method="POST" action="/admin/license/activate">
                    <input type="text" name="licenseKey" placeholder="Lizenzschlüssel eingeben…" required autocomplete="off" style="font-family:monospace;margin-bottom:10px;">
                    <button type="submit" style="background:#dc2626;">Lizenz aktivieren</button>
                </form>
            </div>`;
        }
        const exp = licData?.expiresAt ? new Date(licData.expiresAt) : null;
        const diffDays = exp ? Math.ceil((exp - Date.now()) / 86400000) : null;
        const statusColor  = !exp ? '#059669' : diffDays <= 14 ? '#f59e0b' : '#059669';
        const statusBg     = !exp ? '#f0fdf4' : diffDays <= 14 ? '#fffbeb' : '#f0fdf4';
        const statusBorder = !exp ? '#bbf7d0' : diffDays <= 14 ? '#fcd34d' : '#bbf7d0';
        const expStr = exp ? exp.toLocaleDateString('de-DE', {day:'2-digit',month:'2-digit',year:'numeric'}) : 'Unbegrenzt';
        const daysStr = diffDays !== null && diffDays <= 30 ? ` – noch ${diffDays} Tag${diffDays===1?'':'e'}` : '';
        return `<div class="card">
            <div class="db-card-title">🔑 Lizenz</div>
            ${licOk ? `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:9px 13px;margin-bottom:14px;font-size:13px;color:#059669;font-weight:600;">✅ Lizenz erfolgreich aktiviert!</div>` : ''}
            ${licDeactivated ? `<div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:9px 13px;margin-bottom:14px;font-size:13px;color:#c2410c;font-weight:600;">🔓 Lizenz deaktiviert – Slot ist jetzt frei.</div>` : ''}
            <div style="display:flex;align-items:center;gap:10px;padding:12px 14px;background:${statusBg};border:1px solid ${statusBorder};border-radius:10px;margin-bottom:16px;">
                <span style="font-size:22px;">✅</span>
                <div>
                    <div style="font-weight:700;color:${statusColor};font-size:14px;">Aktiv – gültig bis ${expStr}${daysStr}</div>
                    <div style="font-size:12px;color:var(--muted);margin-top:2px;">Key: <code style="background:var(--border);padding:1px 6px;border-radius:4px;font-size:11px;">${escapeHtml((licData?.key||'').substring(0,8))}••••</code></div>
                </div>
            </div>
            <details style="font-size:13px;color:var(--muted);">
                <summary style="cursor:pointer;font-size:13px;font-weight:600;color:var(--primary);margin-bottom:10px;">Lizenz ersetzen</summary>
                <form method="POST" action="/admin/license/activate" style="margin-top:10px;">
                    <input type="text" name="licenseKey" placeholder="Neuen Lizenzschlüssel eingeben…" required autocomplete="off" style="font-family:monospace;margin-bottom:10px;">
        ...
        <div class="card">
            ...
            ${(() => {
                ...
                if (licenseValid && ld) {
                    ...
                    const expStr = exp ? exp.toLocaleDateString('de-DE', {day:'2-digit',month:'2-digit',year:'numeric'}) : 'Unbegrenzt';
                    return `
                    <div style="display:flex;align-items:center;gap:10px;padding:12px 16px;background:var(--bg);border-radius:8px;border:1px solid var(--border);margin-bottom:12px;">
...
app.get('/admin/srv.js', (_req, res) => {
    ...
    res.send(
        ...
        '    var isKomDesk = type === "komdesk";\n' +
        '    var modal = document.createElement("div");\n' +
        '    modal.className = "srv-modal-bg";\n' +
        '    modal.innerHTML =\n' +
        '        \'<div class="srv-modal">\' +\n' +
        '        \'<div class="srv-modal-icon">\' + (isKomDesk ? "\\u21BA" : "\\u23FB") + \'</div>\' +\n' +
        '        \'<div class="srv-modal-title">\' + (isKomDesk ? "KomDesk neu starten?" : "Linux neu starten?") + \'</div>\' +\n' +
        '        \'<div class="srv-modal-desc">\' +\n' +
        '            (isKomDesk\n' +
        '                ? "Der KomDesk-Dienst wird neu gestartet.<br>Du wirst automatisch weitergeleitet sobald er wieder l\\u00e4uft."\n' +
        ...
        '        \'<span>\' + (isKomDesk ? "Ja, KomDesk jetzt neu starten" : "Ja, Linux-Server jetzt neu starten") + \'</span>\' +\n' +
        '        \'</label>\' +\n' +
        '        \'<div class="srv-modal-btns">\' +\n' +
        '        \'<button class="srv-modal-cancel">Abbrechen</button>\' +\n' +
        '        \'<button class="srv-modal-confirm" id="srv-confirm-btn" disabled style="background:\' + (isKomDesk ? "#f59e0b" : "#dc2626") + \';">Neu starten</button>\' +\n' +
        '        \'</div></div>\';\n' +
        ...
        '    btn.addEventListener("click", function() { modal.remove(); isKomDesk ? srvDoRestart() : srvDoReboot(); });\n' +
        '}\n' +
        'function srvDoRestart() {\n' +
        '    var overlay = srvShowOverlay("KomDesk wird neu gestartet\\u2026", [\n' +
        '        { id: "s1", text: "Neustart-Befehl senden\\u2026" },\n' +
        '        { id: "s2", text: "Server startet neu\\u2026" },\n' +
        '        { id: "s3", text: "Warte bis KomDesk wieder l\\u00e4uft\\u2026" },\n' +
...
app.get('/admin/server/systemctl-status', requireAdmin, requirePermission('server.view'), (req, res) => {
    exec('sudo systemctl status komdesk --no-pager -l', { timeout: 5000 }, (err, stdout, stderr) => {
...
app.post('/admin/server/restart', requireAdmin, requirePermission('server.restart'), requireCsrf, (req, res) => {
    const admin = getCurrentAdmin(req);
    console.log(`[Server] KomDesk-Neustart angefordert von Admin: ${admin?.username || '?'}`);
    res.json({ ok: true });
    exec('sudo systemctl restart komdesk', (err) => {
        if (err) {
        ...
        <div class="card" style="margin-bottom:12px;padding:0;overflow:hidden;">
            ...
            <form method="POST" action="/admin/terminals/update">
                ...
                <div id="trmnl_polling_${escapeHtml(t.id)}" style="display:${(t.trmnlMode||'none')==='polling' ? 'block' : 'none'}">
                    <p style="font-size:13px;opacity:.7;margin-bottom:8px;" data-i18n="terminals.pollingInfo">TRMNL fragt die Daten selbst ab. Kein Push von KomDesk nötig — Intervall direkt in TRMNL einstellen.</p>
                </div>
        ...
        return `
            <div class="card">
                ...
                <div class="links-row">
                    ...
                    <a class="small-link" href="/${encodeURIComponent(room.id)}/api/komdesk" target="_blank" data-i18n="links.openKomDeskApi">KomDesk API öffnen</a>
...
==================================================
*/
app.get('/:room/api/komdesk', (req, res) => {
...
app.post('/admin/update/perform', requireAdmin, requirePermission('update.perform'), requireCsrf, (req, res) => {
    ...
    setTimeout(() => {
        ...
        exec(gitCmd, { timeout: 120000 }, (err, stdout, stderr) => {
            ...
            exec('sudo systemctl restart komdesk', { timeout: 15000 }, (err2) => {
